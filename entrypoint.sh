...
ssh -o StrictHostKeyChecking=no -i id_rsa ${INPUT_SERVER_HOST} "
          ...
          docker run -itd --rm --name ${INPUT_DOCKER_REPO} ${INPUT_DOCKER_OPTIONS} -p ${INPUT_CONTAINER_PORT} ${INPUT_DOCKER_HOST}/${INPUT_DOCKER_NAMESPACE}/${INPUT_DOCKER_REPO}:${INPUT_IMAGE_TAG}"
